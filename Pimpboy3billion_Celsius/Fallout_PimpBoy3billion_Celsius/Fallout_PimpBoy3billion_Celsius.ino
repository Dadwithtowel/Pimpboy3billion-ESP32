...
void cylonEffect() {
  // Define LED states for the Cylon effect
  const int ledCount = 3; // Number of LEDs
  const int delayTime = 50; // Reduced time to stay on each LED (milliseconds)
  const int delayBetweenSweeps = 10; // Reduced time between sweeps (milliseconds)

  // Sweep LEDs from left to right
  for (int i = 0; i < ledCount; i++) {
    digitalWrite(LED_PIN_12 + i, HIGH);
    delay(delayTime);
    digitalWrite(LED_PIN_12 + i, LOW);
    delay(delayBetweenSweeps); // Add small delay between LEDs
  }

  // Sweep LEDs from right to left
...
void loop() {
 timeClient.update();
  ...
  // Read input states
  bool statState = digitalRead(IN_STAT);
  bool invState = digitalRead(IN_INV);
  bool dataState = digitalRead(IN_DATA);

  // Debugging: Print input states to serial monitor
  Serial.print("IN_STAT: ");
  Serial.print(statState);
  Serial.print(" IN_INV: ");
  Serial.print(invState);
  Serial.print(" IN_DATA: ");
  Serial.println(dataState);

  // Control LEDs based on input states
  digitalWrite(LED_PIN_12, statState == LOW ? HIGH : LOW);
  digitalWrite(LED_PIN_13, invState == LOW ? HIGH : LOW);
  digitalWrite(LED_PIN_14, dataState == LOW ? HIGH : LOW);

  // Control screens based on input states
  if (statState == false) {
    flag = 1;
    ...
    while (statState == false) {
      ...
      // Update state to break loop if input changes
      statState = digitalRead(IN_STAT);
    }
  }
  ...
  if (digitalRead(IN_TIME) == false) {
    ...
    // Turn off all LEDs when clock mode is active
    digitalWrite(LED_PIN_12, LOW);
    digitalWrite(LED_PIN_13, LOW);
    digitalWrite(LED_PIN_14, LOW);
  ...
  if (digitalRead(IN_RADIO) == false) {
    ...
    // Run Cylon effect while in radio mode
    while (digitalRead(IN_RADIO) == false) {
      cylonEffect(); // Run the Cylon LED effect
...
void show_hour() {
  ...
  int xpos = 85;
  int ypos = 90; // Top left corner of clock text, about halfway down
  int ysecs = ypos + 24;
